      ...
      <div className="glass-card max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4">
          {MOCK_COMPETENT_PERSONS.map((cp) => (
            <div
              key={cp.id}
              className="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-blue-500/50 transition-all"
            >
              <div className="flex items-start gap-3.5">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-blue-500 to-indigo-600 text-white flex items-center justify-center font-bold text-lg flex-shrink-0 shadow-md">
                  {cp.name.charAt(0)}
                </div>
                <div>
                  ...
                  <p className="text-[11px] font-mono text-slate-500 dark:text-slate-400 mt-1">
                    DOSH Reg No: <strong>{cp.doshRegNo}</strong> | Cert Expiry: {cp.certExpiry}
                  </p>
